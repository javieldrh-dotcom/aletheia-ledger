"""
Fase 2.5 (ampliacion) - Calibracion estadistica con 50 candidatos reales
adicionales (25 confirmados + 25 falsos positivos), excluyendo los 30 ya
usados en la primera iteracion para tener una muestra genuinamente nueva.

Reproducible: semilla fija (43, distinta a la primera ronda para no
resamplear los mismos candidatos por coincidencia).
"""
...
N_PER_CLASS = 25
OUTPUT_DIR = "test-data/calibration_v2"
os.makedirs(OUTPUT_DIR, exist_ok=True)

PREVIOUS_SAMPLE_PATH = "test-data/calibration/sample_manifest.csv"
...
def get_sample():
    ...
    previous = pd.read_csv(PREVIOUS_SAMPLE_PATH)
    excluded_names = set(previous["kepoi_name"])
    print(f"Excluyendo {len(excluded_names)} candidatos ya usados en la primera ronda.")
    ...
    sample_confirmed = confirmed.sample(n=N_PER_CLASS, random_state=43)
    sample_fp = false_pos.sample(n=N_PER_CLASS, random_state=43)

    sample = pd.concat([sample_confirmed, sample_fp]).reset_index(drop=True)
    sample.to_csv(f"{OUTPUT_DIR}/sample_manifest_v2.csv", index=False)
...
def evaluate_candidate(time, flux, flux_err, period, epoch_bkjd, duration_hours):
    """Misma logica de 6 criterios + veto que la primera ronda de calibracion,
    ya con la lógica de filtrado de calidad correcta (solo NaN, sin excluir
    por bandera) validada en la sesion de integracion FITS-en-navegador."""
...
def main():
    ...
    with open(f"{OUTPUT_DIR}/calibration_results_v2.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n\nResultados guardados en {OUTPUT_DIR}/calibration_results_v2.json")
    print_confusion_matrix(results)


def print_confusion_matrix(results):
    valid_results = [r for r in results if "error" not in r]
    print(f"\n=== MATRIZ DE CONFUSION - RONDA 2 ({len(valid_results)} de {len(results)} candidatos procesados) ===")
