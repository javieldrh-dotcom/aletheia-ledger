"""
Fase 2.5 - Calibracion estadistica formal de Aletheia Space.

Descarga una muestra aleatoria estratificada de candidatos Kepler reales
(confirmados y falsos positivos oficiales), corre el motor de vetting de
Aletheia Space sobre cada uno replicando su logica en Python, y construye
una matriz de confusion real contra la disposicion oficial del catalogo.

Reproducible: semilla fija (42), igual que el resto del proyecto.
"""
...
N_PER_CLASS = 15
OUTPUT_DIR = "test-data/calibration"
...
def get_sample():
    """Consulta el catalogo KOI cumulative y selecciona muestra aleatoria estratificada."""
    ...
    # Solo candidatos con parametros completos (necesarios para plegado de fase)
    ...
    print(f"Confirmados disponibles: {len(confirmed)}, Falsos positivos disponibles: {len(false_pos)}")

    sample_confirmed = confirmed.sample(n=N_PER_CLASS, random_state=42)
    sample_fp = false_pos.sample(n=N_PER_CLASS, random_state=42)

    sample = pd.concat([sample_confirmed, sample_fp]).reset_index(drop=True)
    sample.to_csv(f"{OUTPUT_DIR}/sample_manifest.csv", index=False)
    print(f"Muestra guardada: {len(sample)} candidatos ({N_PER_CLASS} + {N_PER_CLASS})")
    return sample


def download_and_flatten(kepoi_name, kepid):
    """Descarga y aplana por trimestre, igual que el metodo validado en Fase 2."""
...
def evaluate_candidate(time, flux, flux_err, period, epoch_bkjd, duration_hours):
    """Replica en Python los 6 criterios del motor TypeScript de Aletheia Space."""
    ...
    # 1. Odd-even symmetry (simplificado: usa numero de ciclo)
    ...
    # 2. Transit shape (flat-bottom fraction)
    if in_transit.sum() > 5:
    ...
    # Combinacion ponderada (misma logica del hook TypeScript)
...
def main():
    ...
    for idx, row in sample.iterrows():
        ...
        try:
            lc = download_and_flatten(kepoi_name, kepid)
            if lc is None:
        ...
        time.sleep(1)  # cortesia con el servidor de MAST

    with open(f"{OUTPUT_DIR}/calibration_results.json", "w") as f:
        json.dump(results, f, indent=2)

    print(f"\n\nResultados guardados en {OUTPUT_DIR}/calibration_results.json")
    print_confusion_matrix(results)


def print_confusion_matrix(results):
    valid_results = [r for r in results if "error" not in r]
    print(f"\n=== MATRIZ DE CONFUSION ({len(valid_results)} de {len(results)} candidatos procesados) ===")
